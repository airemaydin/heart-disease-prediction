...
2. **Data Preprocessing & Cleaning:** 
   ...
   * Scaled numerical features using `MinMaxScaler` for distance-based calculations.
3. **Model Training & Optimization:** 
   * Split the dataset into training and testing sets to evaluate real-world performance.
   * Trained a baseline K-Nearest Neighbors (KNN) classifier.
   * Applied `GridSearchCV` to search for the best hyperparameters (`n_neighbors` and `metric`), finalizing the optimal model.
